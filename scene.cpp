...
void setup_camera(){
	...
	// animate
	transform = std::sin(std::min(2.0,camera.time)*M_PI/4.0);
	t2 = std::min(1.0, 4.0-camera.time);
	camera.p = { 0.2 * (1.1 - transform)/1.1, 0, -3 };
	camera.t.y = { -0.1 * (1.0 - transform), 1, 0 };
	camera.t.z = { -0.1 * (1.1 - transform)/1.1, 0, 1 };
	camera.p -= camera.t.z * transform * 0.5;
	camera.w *= 2, camera.h *= 1.5, camera.c *= 2;
...
color sky(vec d){
	//return vec{-2,0,0};
	return vec{1,1,1} * (d.dot({0,1,0})*0.5+0.5) * (1.25 - transform) / 1.25;
